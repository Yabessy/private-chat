  ...
  return (
    <nav className="flex flex-col items-start z-40 px-4 w-full sticky top-0  font-mono bg-blue-300 text-white">
          ...
          <>
            ...
            {theirProfile && (
              <div className="flex-1 w-auto flex justify-end items-center overflow-hidden">
                ...
                <img src={theirProfile.photoURL || ""} referrerPolicy="no-referrer" alt="userImg" className="rounded-full w-10 h-10 object-contain" />
              </div>
            )}
          </>
        ) : (
      ...
      <ul className="flex space-x-2">
        <li onClick={() => router.push("/")} className={`${whereIsMe === "home" ? `border-b-4 border-white` : ""} cursor-pointer`}>
          Home
        </li>
        <li onClick={() => router.push("/chatroom")} className={`${whereIsMe === "room" ? `border-b-4 border-white` : ""} cursor-pointer`}>
          ChatRoom
        </li>
      </ul>
    </nav>
